...
Result AllocateSharedBufferMemory(std::unique_ptr<Kernel::KPageGroup>* out_page_group,
    ...
    using Core::Memory::EDEN_PAGESIZE;
...
Result MapSharedBufferIntoProcessAddressSpace(Common::ProcessAddress* out_map_address,
    ...
    const VAddr alias_code_size = page_table.GetAliasCodeRegionSize() / EDEN_PAGESIZE;
    ...
    int i;
    for (i = 0; i < 64; i++) {
        *out_map_address = alias_code_begin + ((rng() % alias_code_size) * EDEN_PAGESIZE);
