...
void ShaderCache::Register(std::unique_ptr<ShaderInfo> data, VAddr addr, size_t size) {
    ...
    const u64 page_end = (addr_end + EDEN_PAGESIZE - 1) >> EDEN_PAGEBITS;
    for (u64 page = addr >> EDEN_PAGEBITS; page < page_end; ++page) {
...
void ShaderCache::RemoveEntryFromInvalidationCache(const Entry* entry) {
    const u64 page_end = (entry->addr_end + EDEN_PAGESIZE - 1) >> EDEN_PAGEBITS;
    for (u64 page = entry->addr_start >> EDEN_PAGEBITS; page < page_end; ++page) {
